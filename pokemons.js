...
let section = document.querySelector(`.sectionWrapper`);
let headerWrapper = document.querySelector(`.headerWrapper`);
let select = document.querySelector(`.select`);
let selectweight = document.querySelector(`.select2`)
headerWrapper.appendChild(select);
let input = document.querySelector(`.input`);

// Уникальные типы для выпадающего списка
let uniqueTypes = new Set();

// Динамическое создание карточек покемонов
pokemons.forEach((e) => {
	const div = document.createElement("div");
	div.classList.add("cards"); // Добавляем класс 'cards'
	div.innerHTML = `
		<div class="card bg-yellow-300 pt-[10px] pl-[10px] pr-[10px] pb-[10px] rounded-2xl flex flex-col justify-center items-center">
			<div class="nameCArd">
				<h1>${e.name}</h1>
				<p>${e.num}</p>
			</div>
			...
			<div class="flex gap-[10px]">
				${e.type.map((type) => `<h2>${type}</h2>`).join("")}
	...
	// Добавляем типы в Set для уникальности
	e.type.forEach((type) => uniqueTypes.add(type));
	
});

// Добавляем "Все" как опцию в выпадающий список
let allOption = document.createElement("option");
allOption.value = ""; // Пустое значение для сброса фильтра
allOption.textContent = "Все";
select.append(allOption);

// Добавляем уникальные типы в выпадающий список
...
function filterCards() {
	let inputValue = input.value.toLowerCase(); // Значение из поля ввода
	let selectedType = select.value.toLowerCase(); // Выбранный тип из выпадающего списка

	document.querySelectorAll(".cards").forEach((card) => {
		let nameCArd = card.querySelector(".nameCArd");
		let name = nameCArd.querySelector("h1").textContent.toLowerCase();
		let num = nameCArd.querySelector("p").textContent.toLowerCase();
		let types = Array.from(card.querySelectorAll("h2")).map((h2) =>
			h2.textContent.toLowerCase()
		);

		// Проверяем соответствие ввода и типа
		let matchesInput =
			!inputValue || name.includes(inputValue) || num.includes(inputValue);
		let matchesType = !selectedType || types.includes(selectedType);

		// Показываем или скрываем карточку
		if (matchesInput && matchesType) {
